...
The output above has two sections: a `pyproject.toml` dependency table (from `versioning.libs`) and a
...
- `pyproject.toml`: this only rewrites version constraints — it does NOT install anything. If the
  user says yes, run `uv run --no-sync invoke versioning.libs --yes`, then tell them the locks were
...
- `.github/workflows/`: this only rewrites `@ref` pins in the workflow files — it does NOT run any
  workflow. If the user says yes, run `uv run --no-sync invoke versioning.workflows --yes`, then tell
