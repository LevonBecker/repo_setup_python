...
steps), so the module stays a pure file operation, easy to reason about and lint independently of
git state.
...
raw push, not bypassing the rule. `main` has no such rule (only `promote.yml` pushes there,
...
current HEAD. For a dev deploy, `bump-version` pushes a new commit to `development` in an earlier
